...
// --- Progress rollup (computed alongside the sort entries) ---
    ...
    for (issues) |issue| {
        ...
        if (status == null) {
            ...
            partial_count += 1;
        } else if (std.mem.eql(u8, status.?, "Blocked")) {
            blocked_count += 1;
        } else {
            spec_only_count += 1;
        }
    ...
    try index_writer.interface.print(
        "<!-- generated by weave — reading order derived from title section numbers -->\n\n" ++
            ...
            "## Progress\n\n" ++
            "✅ Implemented: {d}  |  🟡 Partial: {d}  |  🔴 Blocked: {d}  |  ⬛ Spec only: {d}  |  ⬜ Not started: {d}  |  Total: {d}\n\n",
        .{ implemented_count, partial_count, blocked_count, spec_only_count, not_started_count, issues.len },
    );
